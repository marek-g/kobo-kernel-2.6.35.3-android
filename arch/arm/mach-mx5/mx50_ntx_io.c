...
void mxc_set_fl_duty (int duty)
{
	...
	if(0==gptHWCFG->m_val.bFrontLight || duty == 100)
		return;
...
#else
       // Antony edit 20130128 : patch from Taipei
...
/*
static irqreturn_t FL_key_int(int irq, void *dev_id)
{
	printk("========= FL =========\n");
	schedule_delayed_work(&FL_key_func, 20);
	return 0;
}
*/
...
int ntx_charge_status (void)
{
	if ((10 == check_hardware_name() || 5 == check_hardware_name() || 30 ==check_hardware_name()) && !g_pwr_key ) {//E606A2, E606E2 or E60672
#ifdef NTX_BAT_CHG_SW_SIM
		if (gpio_get_value (GPIO_ACIN_PG)) {
       	...
       	return 3;
#else
        //return (gpio_get_value (GPIO_ACIN_PG)?0:3);
...
static irqreturn_t msp_int(int irq, void *dev_id)
{
	...
	printk ("[%s-%d] MSP430 interrupt triggered usb=%d!!!\n",__func__,__LINE__,usbStat);
	gIsMSP430IntTriggered = 1;
	ntx_charger_online_event_callback ();	// update charge / battery status
	if (gBatteryPercentage <= 1 && gSleep_Mode_Suspend && usbStat==1)
...
/*
	...
	gpio_request(GPIO_ACT_ON, "action_on");
	gpio_direction_input(GPIO_ACT_ON);
	{
		irq = gpio_to_irq(GPIO_ACT_ON);
		printk("chk_hw %d\n", check_hardware_name());
		if ((6 == check_hardware_name()) || (2 == check_hardware_name()) ) 		// E60632 || E50602 || E606E2
			set_irq_type(irq, IRQF_TRIGGER_RISING);
		else
			set_irq_type(irq, IRQF_TRIGGER_FALLING);

		ret = request_irq(irq, FL_key_int, 0, "action_on", 0);
		if (ret)
			pr_info("register on-off key interrupt failed\n");
		else
			enable_irq_wake(irq);
...
//	if ((1 == check_hardware_name()) || (10 == check_hardware_name()) || (14 == check_hardware_name())) {	// E60612 , E606A2 ,E606B2 , ir touch
	...
	if(0!=gptHWCFG->m_val.bFrontLight) {
		...
		//INIT_DELAYED_WORK(&FL_key_func, ISR_FL_key_func);
	}
...
//	gpio_direction_output(GPIO_WIFI_RST,1);
	...
	//Terry add for EPD white screen problem
	gpio_direction_output(GPIO_WIFI_INT,0);
...
//	if (gSleep_Mode_Suspend && (1 != check_hardware_name()) && (10 != check_hardware_name()) && (14 != check_hardware_name())) {
   ...
   //Terry: we dont want to disable wifi power while we dont unload driver
	//gpio_direction_input(GPIO_WIFI_3V3);
	//gpio_direction_output(GPIO_WIFI_RST,0);
#endif
